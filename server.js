// ═══════════════════════════════════════════════════════════
//  Bodhisatvvam — Backend Server v5
//
//  Changes from v4:
//    ✅ scriptSrcAttr: 'unsafe-inline' — fixes onclick= buttons blocked by CSP
//    ✅ cdn.razorpay.com added to CSP — fixes Razorpay risk-detection script blocked
//    ✅ Server-side price recalculation via PRODUCT_CATALOG — prevents price manipulation
//    ✅ Full try/catch around every route — no more HTML 500 responses
//    ✅ Global JSON error handlers — body-parse errors and unhandled throws return JSON
//    ✅ Stale v4 strings, duplicate section numbers cleaned up
// ═══════════════════════════════════════════════════════════
...
// ── 5. PRODUCT CATALOG (server-side pricing source of truth) ─────────────────
// SECURITY: Frontend sends item IDs + quantities only. Backend recalculates the
// total from this catalog. Any client-supplied total or price is fully ignored.
...
function formatItemsForWhatsApp(items) {
    ...
    return items.map(i => {
        let line = `- ${i.name} x${i.qty} @ ₹${i.price}`;
        if (i.notes) line += `\n  _Note: ${i.notes}_`;
        return line;
    }).join('\n');
}
...
app.post('/api/create-order', orderLimiter, async (req, res) => {
    try {
        ...
        // SECURITY: Recalculate total from catalog — never trust any price from the client
...
app.post('/api/verify-payment', orderLimiter, async (req, res) => {
    try {
        ...
        const {
            ...
            bodhiOrderId, name, phone, address, items, total,
        } = body;
        ...
        const sheetOk = await pushToSheet({
            ...
            address, items, total, status: 'Paid', paymentId: razorpay_payment_id,
        });

        const waOk = await sendWhatsApp(name, sanitizedPhone, bodhiOrderId, items, total, address);

        // Safety net — if both notifications fail, log order to stdout so Render retains it
        if (!sheetOk && !waOk) {
            ...
            console.error(`🚨 FULL ORDER DATA (backup): ${JSON.stringify({
                orderId: bodhiOrderId, paymentId: razorpay_payment_id,
                name, phone: sanitizedPhone, address, items, total,
                timestamp: new Date().toISOString(),
            })}`);
        }

        // Payment IS confirmed regardless of notification status
        return res.status(200).json({
            success: true,
...
// Catches body-parse failures from express.json() — returns JSON not HTML
...
// Catch-all — every unhandled throw returns JSON, never the default Express HTML page
